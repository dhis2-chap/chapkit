"""Config service factory with seeding and custom metadata, used as a fixture by integration tests."""
...
SEED_CONFIGS: tuple[tuple[str, ULID, EnvironmentConfig], ...] = (
    (
        ...
        EnvironmentConfig(debug=False, api_host="0.0.0.0", api_port=8080, max_connections=2000),
    ),
    (
        "staging",
        ULID.from_str("01K72P5N5KCRM6MD3BRE4P07N9"),
        EnvironmentConfig(debug=True, api_host="127.0.0.1", api_port=8001, max_connections=500),
    ),
    (
        "local",
        ULID.from_str("01K72P5N5KCRM6MD3BRE4P07NA"),
        EnvironmentConfig(debug=True, api_host="127.0.0.1", api_port=8000, max_connections=100),
    ),
)
...
async def seed_configs(app: FastAPI) -> None:
    ...
    async with database.session() as session:
        ...
        manager = ConfigManager[EnvironmentConfig](repository, EnvironmentConfig)
...
def build_config_app() -> FastAPI:
    """Build a config CRUD service with seeded environments and extended service info."""
